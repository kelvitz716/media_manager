...
class TelegramDownloader:
    ...
    async def _process_media(self, message: Message) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Message details - Chat ID: {message.chat.id}, Message ID: {message.message_id}")
            ...
            async with self._download_lock:
                ...
                self.logger.debug(f"Initialized download tracking for {filename}")
            ...
            self.logger.debug(f"Retrieving message {message.message_id} through Telethon")
            ...
            self.logger.debug(f"Download path: {filepath}")
            ...
            async def progress_callback(received_bytes, total):
                if total:
                    ...
                    self.logger.debug(f"Download progress for {filename}: {percentage:.1f}% ({received_bytes}/{total} bytes)")

            self.logger.info(f"Starting media download to {filepath}")
            ...
            self.logger.info(f"Successfully downloaded {filename} to {filepath}")
            ...
            async with self._download_lock:
                if file_id in self._active_downloads:
                    ...
                    self.logger.debug(f"Removed download tracking for {filename}")

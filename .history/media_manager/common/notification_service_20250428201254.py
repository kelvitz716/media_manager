...
class NotificationService:
    ...
    async def start(self) -> None:
        ...
        if self.config_manager["notification"]["enabled"]:
    ...
    async def _initialize_bot(self) -> None:
        ...
        if self.config_manager["notification"]["method"] != "telegram":
            return

        bot_token = self.config_manager["notification"]["bot_token"]
        if not bot_token:
    ...
    async def notify(self, message: str, level: str = "info", wait_response: bool = False, response_timeout: float = 300) -> Optional[str]:
        ...
        try:
            ...
            chat_id = self.config_manager["notification"]["chat_id"]

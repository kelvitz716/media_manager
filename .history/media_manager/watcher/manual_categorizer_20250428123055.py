...
from .categorizer import MediaCategorizer
...
class ManualCategorizer:
    ...
    async def _get_movie_details(self, file_path: str) -> None:
        ...
        try:
            ...
            # Ask for movie title
            title_prompt = (
                f"Enter movie title for: {filename}\n"
            ...
            # Ask for year
            year_prompt = "Enter the movie's release year:"
            year = await self.notification.wait_for_response(year_prompt)
            if not year:
                return

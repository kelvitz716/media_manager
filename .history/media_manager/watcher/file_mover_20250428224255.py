...
class MediaFileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event: FileCreatedEvent) -> None:
        ...
        if not event.is_directory:
            self.logger.debug(f"File creation detected: {event.src_path}")
    ...
    async def _handle_new_file(self, file_path: str) -> None:
        ...
        try:
            ...
            async with self._processing_lock:
                ...
                self.logger.debug(f"Started processing file: {file_path}")
            
            try:
                # Wait for file to be fully written
                self.logger.debug(f"Waiting for file to be ready: {file_path}")

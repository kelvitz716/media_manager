...
class MediaWatcher:
    ...
    def __init__(self, config: Dict[str, Any], categorizer: MediaCategorizer, notification: NotificationService):
        """Initialize watcher.
        
        Args:
            config: Configuration dictionary
            categorizer: Media categorizer instance
            notification: Notification service instance
        """
        self.config = config
    ...
    async def _handle_media_file(self, file_path: str) -> None:
        """Handle a new media file.
        ...
        try:
            ...
            result = await self.categorizer.process_file(file_path)
            if not result:
                await self.categorizer.move_to_unmatched(file_path)
        ...
        for root, _, files in os.walk(watch_dir):
            for file in files:
                if not self._running:
                    break
                await self._handle_media_file(os.path.join(root, file))
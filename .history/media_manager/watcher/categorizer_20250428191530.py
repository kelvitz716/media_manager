...
class MediaCategorizer:
    ...
    async def process_movie(self, file_path: str) -> bool:
        ...
        try:
            ...
            results = await self.tmdb.search_movie(title, int(year))
            ...
            await self.notification.notify(
                f"Processed movie: {movie_info['title']} ({movie_info['year']})",
                level="info"
            )
            return True
    ...
    async def process_tv_show(self, file_path: str) -> bool:
        ...
        try:
            ...
            if not episode_info or not episode_info.get("episode_number"):
                await self.notification.notify(

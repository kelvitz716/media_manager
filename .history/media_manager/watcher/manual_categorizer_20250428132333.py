...
class ManualCategorizer:
    ...
    async def _start_categorization(self, file_path: str) -> None:
        ...
        try:
            ...
            async with self._session_lock:
                self._active_sessions[file_path] = session_data
            
            # Ask for media type
            prompt = (
                f"Starting categorization for: {filename}\n\n"
            ...
            await self.notification.notify(prompt, level="info")
    ...
    async def _process_file(self, file_path: str) -> None:
        ...
        try:
            ...
            # Remove type from metadata before passing to categorizer
            process_metadata = {k: v for k, v in metadata.items() if k != "type"}
            
            if media_type == "movie":
                success = await self.categorizer._process_movie(file_path, process_metadata)
            else:
                success = await self.categorizer._process_tv_show(file_path, process_metadata)
            ...
            if success:
                ...
                # Finally check for remaining files
                unmatched = self._get_unmatched_files()
                if unmatched:
                    # Add a small delay to ensure notifications arrive in order

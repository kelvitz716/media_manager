...
class MediaFileHandler(FileSystemEventHandler):
        ...
        try:
            ...
            try:
                ...
                if not os.path.exists(file_path):
                    self.logger.warning(f"File no longer exists after stabilization: {file_path}")
                    return
                ...
                if success:
                    ...
                    self.logger.warning(f"Failed to categorize file: {file_path}")
    ...
    async def _wait_for_file_ready(self, file_path: str, timeout: int = 300) -> None:
        ...
        """
        self.logger.debug(f"Waiting for file to stabilize: {file_path}")
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if current_size == last_size:
                    self.logger.debug(f"File size stabilized at {current_size} bytes")
                    return

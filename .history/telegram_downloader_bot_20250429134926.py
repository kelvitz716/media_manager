...
class DownloadManager:
    ...
    def __init__(self, bot, config, client_manager=None):
        """
        ...
        self.categorizer = MediaCategorizer(config)
    ...
    async def _download_file(self, task: DownloadTask) -> None:
        ...
        try:
            ...
            if success:
                ...
                # Trigger categorization after successful download
                try:
                    await self.categorizer.process_file(task.file_path)
                except Exception as e:
                    logger.error(f"Error categorizing file: {str(e)}")
                    await self.bot.send_message(
                        chat_id=task.chat_id,
                        text=f"✅ Download complete but categorization failed. You may need to use /categorize command."
                    )
                    return
                ...
                if not task.batch_id:
                    await self.bot.send_message(
                        chat_id=task.chat_id,
                        text=f"✅ Download complete! File has been processed and will appear in your Jellyfin library."
                    )
            else:
    ...
    def _setup_handlers(self) -> None:
        ...
        async def handle_media(message):
            ...
            ack_message = await self.bot.send_message(
                ...
                f"Status: ⏳ Added to download queue (Position: {len(self.download_manager.download_queue) + 1})\n\n"
...
class TelegramDownloader:
    ...
    async def _startup(self) -> None:
        ...
        try:
            ...
            logger.error(f"Startup errorpython
: {str(e)}")
            self.running = False

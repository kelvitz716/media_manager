...
class TestTelegramDownloader(IsolatedAsyncioTestCase):
    ...
    async def test_process_media_document(self):
        ...
        # Mock download_file to return an async iterator
        self.downloader.bot.download_file.return_value = async_iter([b"test_data"])
    ...
    async def test_duplicate_download(self):
        ...
        await message.reply_to.assert_awaited_with(
    ...
    async def test_download_progress(self):
        ...
        self.downloader.bot.download_file.return_value = async_iter(chunks)
        ...
        progress_calls = [
            mock_call for mock_call in self.notification.notify.call_args_list
            if "Downloading test.mp4:" in mock_call.args[0]
        ]

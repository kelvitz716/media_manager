...
async def test_notify_with_reply(notification_service):
    ...
    async def send_message(*args, **kwargs):
        ...
        response_future = notification_service.wait_for_response(msg.message_id)
...
async def test_multiple_responses(notification_service):
    ...
    received_responses = []
    received_responses.append(await notification_service.notify("message 1", wait_response=True))
    received_responses.append(await notification_service.notify("message 2", wait_response=True))

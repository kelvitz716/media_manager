...
class TestManualCategorizer(IsolatedAsyncioTestCase):
    ...
    async def test_continue_iteration(self):
        ...
        self.categorizer._process_movie.assert_called_with(
            test_files[0],
            {"title": "Test Movie 1", "year": 2024}
        )
        
        # Verify notifications about remaining files
        self.notification.notify.assert_has_calls([
            mock.call(
            ...
            mock.call(
                "2 files remaining to categorize",  # Updated to match actual count
                level="info"
            )
        ], any_order=False)

        # Reset mock for next file
        ...
        # Verify second file was processed
        self.categorizer._process_movie.assert_called_with(
            test_files[1],
        ...
        self.notification.notify.assert_has_calls([
            ...
            mock.call(
                "1 file remaining to categorize",  # This is correct since we had 3 files total
                level="info"
            )
        ], any_order=False)